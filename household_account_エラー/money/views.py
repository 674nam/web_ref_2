...
from django.contrib.auth.decorators import login_required # ログインユーザーのみ操作可能
...
class PaymentCreate(LoginRequiredMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        # ログインしているユーザーのaccount_idを取得
        account_id = self.request.user.account_id
        # フォームのデータを取得し、支出レコードにログインユーザーのaccount_idを追加して保存
        self.object = form.save(commit=False)
        self.object.account_id = account_id
        self.object.save()
        messages.info(self.request,
                        f'支出を登録しました\n'
                        f'日付: {self.object.date}\n'
                        f'カテゴリ: {self.object.category}\n'
                        f'金額: {self.object.price}円')
